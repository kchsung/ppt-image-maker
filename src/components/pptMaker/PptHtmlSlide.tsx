  ...
  const content = (
    <div className={className} data-pptx-slide={exportMode ? String(slide.pageNumber) : undefined} style={exportMode ? surfaceStyle : { ...surfaceStyle, transform: `scale(${previewScale})` }}>
      ...
      <div style={{ position: 'absolute', left: 116, top: 72, width: 1260 }}>
        <div style={{ fontSize: getTitleFontSize(slide.title), fontWeight: 800, lineHeight: 1.1 }}>{slide.title}</div>
      ...
      <div style={{ position: 'absolute', left: 74, bottom: 78, width: 1450, fontSize: getFooterFontSize(slide.takeaway, 19), lineHeight: 1.25, color: MUTED, overflowWrap: 'anywhere' }}>{slide.takeaway}</div>
      <div style={{ position: 'absolute', left: 74, bottom: 40, width: 1450, fontSize: getFooterFontSize(decision, 21), fontWeight: 800, lineHeight: 1.2, overflowWrap: 'anywhere' }}>{decision}</div>
...
function ContentCard({ block, index, numbered = false, compact = false }: { block: SlideContentBlock; index: number; numbered?: boolean; compact?: boolean }) {
  return <div style={{ flex: 1, minWidth: 0, minHeight: compact ? 132 : 194, border: `2px solid ${index % 2 === 0 ? '#CFE0FA' : '#FFD9BC'}`, borderRadius: 16, background: index % 2 === 0 ? PALE_BLUE : PALE_ORANGE, padding: compact ? '18px 20px' : '22px 24px', display: 'flex', flexDirection: 'column', gap: compact ? 10 : 14, overflow: 'hidden' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><div style={{ flex: '0 0 auto', width: 34, height: 34, borderRadius: 999, background: index % 2 === 0 ? NAVY : ACCENT, color: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 800 }}>{numbered ? String(index + 1).padStart(2, '0') : String(index + 1)}</div><div style={{ fontSize: getCardHeadingFontSize(block.heading, compact), fontWeight: 800, lineHeight: 1.16, overflowWrap: 'anywhere' }}>{block.heading}</div></div>
    <div style={{ fontSize: getCardDetailFontSize(block.detail, compact), color: MUTED, lineHeight: 1.36, overflowWrap: 'anywhere' }}>{block.detail}</div>
