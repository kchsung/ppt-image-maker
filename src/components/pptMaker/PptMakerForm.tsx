...
import type { PptMakerFormState, TargetLanguage } from '@/types/models/pptMaker.model';
...
export function PptMakerForm({ form, isLoading, onChange, onSubmit }: PptMakerFormProps) {
  const canSubmit = form.sourceText.trim().length >= 40 && !isLoading;
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          <span className="mb-1 block text-sm font-semibold text-text-main">Sample slide image</span>
          <div className="rounded-md border border-dashed border-border bg-surface-muted p-3">
            {form.styleImageDataUrl ? (
              <div className="flex items-center gap-3">
                ...
                <Button variant="ghost" onClick={() => onChange({ styleImageDataUrl: null })} aria-label="Remove sample image">
                  <X className="h-4 w-4" />
                </Button>
              </div>
            ) : (
